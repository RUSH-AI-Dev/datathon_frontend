...
const Bangkok = require("./models/Bangkok");
...
router.get("/Bangkok", async (req, res) => {
  const result = await Bangkok.findAll({ order: Sequelize.literal("id DESC") });
  res.json(result);
});
...
router.post("/Bangkok", (req, res) => {
  try {
    const form = new formidable.IncomingForm();
    form.parse(req, async (error, fields, files) => {
      let result = await Bangkok.create(fields);
...
router.put("/Bangkok", (req, res)=>{
  try {
    const form = new formidable.IncomingForm();
    form.parse(req, async (error, fields, files) => {
      let result = await Bangkok.update(fields, {where : {id: fields.id}});
...
router.delete("/Bangkok/id/:id", async (req, res)=>{
  try{
    const {id} = req.params
    let result = await Bangkok.findOne({where: {id}})
    await fs.remove(__dirname + "/uploaded/images/" + result.image)
    result = await Bangkok.destroy({ where: { id: id } });
